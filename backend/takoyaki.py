...
pictures = []
fps = 30
img1 = cv2.imread("octopus.png")
bg_img = cv2.imread("takoyaki.jpeg")
...
for i in range(fps):
    if i%fps <= fps//2:
        ...
        pic = Image.open(loc)
        pictures.append(pic)
        pictures[0].save('Takoyaki.gif',save_all=True, append_images=pictures[1:], optimize=False, duration=100, loop=0)

    else:
        bg_img = cv2.imread("takoyaki.jpeg")
        loc = 'tpic' + str(i)+'.png'
        x, y = 1300, 1600-7*(fps-i)
        w = min(fg_img.shape[1], bg_img.shape[1] - x)
        h = min(fg_img.shape[0], bg_img.shape[0] - y)
        fg_roi = fg_img[:h, :w]
        bg_roi = bg_img[y : y + h, x : x + w]
